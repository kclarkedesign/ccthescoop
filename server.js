...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.comment && requestComment.comment.body && database.articles[requestComment.comment.articleId] &&
...
//Persistant Scoop database
function loadDatabase() {
  const readYaml = require('read-yaml');

  readYaml('./src/memory/scoopDB.yaml', function(err, yamlDatabase) {
    if (err) {
      throw err;
    }

    database.users = yamlDatabase.users;
    database.articles = yamlDatabase.articles;
    database.nextArticleId = yamlDatabase.nextArticleId;
    database.comments = yamlDatabase.comments;
    database.nextCommentId = yamlDatabase.comments;
...
function saveDatabase() {
  ...
  writeYaml('./src/memory/scoopDB.yaml', database, function(err) {
    if (err) {
      throw err;
    }
  });
